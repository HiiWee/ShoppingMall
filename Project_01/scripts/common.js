...
--------------------------------------------------------------------------- */
/* 이이디 찾기 : 인증번호 받기 버튼 */ 
function certificationNumReceive() {
  var findInfoName = document.getElementById("findInfoName").value;
  var findInfoMobile = document.getElementById("findInfoMobile").value;
  ...
  // 이름체크
  if (findInfoName.length == 0) {
    alert("이름을 입력하세요");
    return; // exit 기능, return만 사용하면 null 반환.
  } 
  else if (findInfoName.search(/\s/) != -1) {
    alert("이름에 공백은 들어갈 수 없습니다.");
    return; // exit 기능, return만 사용하면 null 반환.
  } 
  else if (patternChk1.test(findInfoName)) {
    alert("이름에 특수문자는 들어갈 수 없습니다.");
    return; // exit 기능, return만 사용하면 null 반환.
  }
  // 휴대폰번호 체크
  else if (findInfoMobile.length == 0 || patternChk1.test(findInfoMobile) || !patternChk2.test(findInfoMobile) || patternChk3.test(findInfoMobile) || findInfoMobile.search(/\s/) != -1) {
...
function findIdChk() {
  ...
  // 인증번호 체크
  else if (certificationNum.length == 0) {
    alert("인증번호가 다릅니다.");
    return; // exit 기능, return만 사용하면 null 반환.
  }
...
/* 비밀번호 찾기 : 인증번호 받기 버튼 */
function certificationNumReceive2() {
  var findInfoId = document.getElementById("findInfoId").value;
  var findInfoName = document.getElementById("findInfoName").value;
  var findInfoMobile = document.getElementById("findInfoMobile").value;
  ...
  // 아이디 체크
  if (findInfoId.length == 0) {
    alert("아이디를 입력해 주십시오.");    
    return; //exit 기능, return만 사용하면 null반환.
  }
  else if (findInfoId.search(/\s/) != -1) {
    alert("아이디에 공백은 들어갈 수 없습니다.");    
    return; //exit 기능, return만 사용하면 null반환.
  }
  else if (patternChk1.test(findInfoId)) {
    alert("아이디에 특수문자는 들어갈 수 없습니다.");    
    return; //exit 기능, return만 사용하면 null반환.
  }
  
  // 이름 체크
  else if (findInfoName.length == 0) {
    alert("이름을 입력해 주십이오.");    
    return; //exit 기능, return만 사용하면 null반환.
  }
  else if (findInfoName.search(/\s/) != -1) {
